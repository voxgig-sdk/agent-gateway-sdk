...
boru-driven command-line client **and** interactive REPL for the AgentGateway
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/agent-gateway-cli)
make build

# 2. See usage (words, entities, env vars)
./agent-gateway-cli --help

# 3. Provide credentials once, via the environment
export AGENT_GATEWAY_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./agent-gateway-cli load 1 analytics            # {id:1} shorthand
./agent-gateway-cli load '{id:1}' analytics       # explicit match map

# 5. Override the API base URL for a single call
AGENT_GATEWAY_BASE=https://api.example.com ./agent-gateway-cli load 1 analytics

# 6. No arguments -> interactive REPL
./agent-gateway-cli
agent-gateway> load 1 analytics
agent-gateway> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/agent-gateway-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/agent-gateway-cli load 1 analytics
   ```
...
```sh
./agent-gateway-cli load 1 analytics          # scalar shorthand for {id:1}
./agent-gateway-cli load '{id:1}' analytics     # explicit match map
```
...
./agent-gateway-cli load 1 analytics
```
...
Run with no arguments to open a REPL (prompt `agent-gateway>`). Each line is
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/agent-gateway-cli`. |
...
(`./agent-gateway-cli <expr>`) and interactively (the REPL), and expressions compose the
