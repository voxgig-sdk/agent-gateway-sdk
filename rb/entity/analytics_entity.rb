# AgentGateway SDK Analytics entity
...
class AnalyticsEntity
  ...
  def make
    opts = @_entopts.dup
    AnalyticsEntity.new(@_client, opts)
  end
  ...
  #
  # @param reqmatch [AnalyticsLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.Analytics.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [Analytics, Hash] the loaded Analytics; raises AgentGatewayError on failure
