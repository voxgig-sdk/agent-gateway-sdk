# AgentGateway Ruby SDK

The Ruby SDK for the AgentGateway API. Provides an entity-oriented interface using idiomatic Ruby conventions.
...
```ruby
result, err = client.Analytics(nil).load({ "id" => "example_id" }, nil)
raise err if err
puts result
```
...
result, err = client.AgentGateway(nil).load(
  { "id" => "test01" }, nil
)
