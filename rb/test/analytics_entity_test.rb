...
class AnalyticsEntityTest < Minitest::Test
  ...
  def test_basic_flow
    setup = analytics_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["load"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "analytics." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set AGENT_GATEWAY_TEST_ANALYTICS_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    analytics_ref01_data = nil
    if analytics_ref01_data_raw.length > 0
...
def analytics_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "analytics", "AnalyticsTestData.json")
  ...
  entid_env_raw = ENV["AGENT_GATEWAY_TEST_ANALYTICS_ENTID"]
  idmap_overridden = !entid_env_raw.nil? && entid_env_raw.strip.start_with?("{")

  env = Runner.env_override({
    "AGENT_GATEWAY_TEST_ANALYTICS_ENTID" => idmap,
  ...
  idmap_resolved = Helpers.to_map(
    env["AGENT_GATEWAY_TEST_ANALYTICS_ENTID"])
