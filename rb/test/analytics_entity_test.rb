...
class AnalyticsEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set AGENTGATEWAY_TEST_ANALYTICS_ENTID JSON to run live"
      return
    end
    client = setup[:client]
...
def analytics_basic_setup(extra)
  ...
  entid_env_raw = ENV["AGENTGATEWAY_TEST_ANALYTICS_ENTID"]
  idmap_overridden = !entid_env_raw.nil? && entid_env_raw.strip.start_with?("{")

  env = Runner.env_override({
    "AGENTGATEWAY_TEST_ANALYTICS_ENTID" => idmap,
  ...
  idmap_resolved = Helpers.to_map(
    env["AGENTGATEWAY_TEST_ANALYTICS_ENTID"])
