...
[MCP](https://modelcontextprotocol.io) server exposing the AgentGateway SDK as
two agent tools — `agent-gateway_list` and `agent-gateway_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/agent-gateway-mcp)
make build

# 2. Provide credentials via the environment
export AGENT_GATEWAY_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user agent-gateway \
  -- /absolute/path/to/agent-gateway-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./agent-gateway-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// agent-gateway_list: first page of records
{ "entity": "service" }
{ "entity": "service", "query": { } }

// agent-gateway_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/agent-gateway-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user agent-gateway \
     -- "$PWD"/dist/*/agent-gateway-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `agent-gateway_list` and `agent-gateway_load` tools now appear
   in new sessions. Ask the agent to *"list service using agent-gateway"*
   and it calls `agent-gateway_list` with `{"entity":"service"}`.
...
```sh
export AGENT_GATEWAY_APIKEY=sk_live_xxx            # API key
export AGENT_GATEWAY_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `agent-gateway_list` tool
...
### Call the `agent-gateway_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `agent-gateway_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `agent-gateway_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `AGENT_GATEWAY_APIKEY` | API key sent with every request. |
| `AGENT_GATEWAY_BASE` | Optional override of the API base URL. |

### Entities

The 6 entities valid as the `entity` argument:

analytics | api_key | balance | meta | payment | service

### Smoke test via HTTP (raw JSON-RPC)

```sh
./agent-gateway-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"agent-gateway_load","arguments":{"entity":"analytics","query":{"id":1}}}}'
```
