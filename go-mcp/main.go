...
func main() {
	...
	// Configure from the environment: AGENT_GATEWAY_APIKEY carries the API key and
	// AGENT_GATEWAY_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("AGENT_GATEWAY_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("AGENT_GATEWAY_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewAgentGatewaySDK(opts)
