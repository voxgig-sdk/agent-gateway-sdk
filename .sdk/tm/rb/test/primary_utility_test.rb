...
class PrimaryUtilityTest < Minitest::Test
  ...
  def test_feature_add_basic
    ...
    feature = AgentGatewayBaseFeature.new
  ...
  def test_feature_hook_basic
    hook_client = AgentGatewaySDK.test(nil, nil)
  ...
  def test_feature_init_basic
    init_client = AgentGatewaySDK.test(nil, nil)
  ...
  def test_fetcher_live
    calls = []
    live_client = AgentGatewaySDK.new({
      "system" => {
        "fetch" => ->(url, fetchdef) {
  ...
  def test_fetcher_blocked_test_mode
    blocked_client = AgentGatewaySDK.new({
      "system" => {
        "fetch" => ->(url, fetchdef) {
          return {}, nil
        },
  ...
  def test_make_result_no_result
    ctx = make_test_full_ctx(@client, @utility)
    ctx.spec = AgentGatewaySpec.new({ "step" => "start" })
    ctx.result = nil
  ...
  def test_make_spec_basic
    ...
    spec_client = AgentGatewaySDK.test(nil, setup_opts)
  ...
  def test_make_url_basic
    runset(get_spec(@primary, "makeUrl", "basic")) do |entry|
      ctxmap = entry["ctx"] || {}
      ctx = make_ctx_from_map(ctxmap, @client, @utility)
      ctx.result = AgentGatewayResult.new({}) unless ctx.result
  ...
  def test_operator_basic
    runset(get_spec(@primary, "operator", "basic")) do |entry|
      in_val = entry["in"] || {}
      op = AgentGatewayOperation.new(in_val)
      {
        "entity" => op.entity,
        "name" => op.name,
        "input" => op.input,
  ...
  def test_prepare_auth_basic
    ...
    auth_client = AgentGatewaySDK.test(nil, setup_opts)
  ...
  def make_ctx_from_map(ctxmap, client, utility)
    ...
    ctx = AgentGatewayContext.new(ctxmap, nil)
    ...
    if ctxmap["spec"].is_a?(Hash)
      ctx.spec = AgentGatewaySpec.new(ctxmap["spec"])
    end

    # Handle result from JSON map
    if ctxmap["result"].is_a?(Hash)
      res_map = ctxmap["result"]
      ctx.result = AgentGatewayResult.new(res_map)
      if res_map["err"].is_a?(Hash) && res_map["err"]["message"].is_a?(String)
        ctx.result.err = AgentGatewayError.new("", res_map["err"]["message"])
      end
    end
    ...
    if ctxmap["response"].is_a?(Hash)
      ...
      ctx.response = AgentGatewayResponse.new(resp_map)
      if resp_map["body"]
  ...
  def err_from_map(m)
    ...
    code = m["code"] || ""
    AgentGatewayError.new(code, msg)
  end
end
