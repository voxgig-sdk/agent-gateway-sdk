...
module AgentGatewayUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = AgentGatewayResponse.new({})
    ...
    if fetch_err
      ...
      response = AgentGatewayResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = AgentGatewayResponse.new(fetched)
    else
