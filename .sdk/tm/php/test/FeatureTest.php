...
class FtCtrl extends AgentGatewayControl
{
...
class FtClient
{
    ...
    private AgentGatewayUtility $utility;

    public function __construct(AgentGatewayUtility $utility, array $options)
    {
    ...
    public function get_utility(): AgentGatewayUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (AgentGatewayContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public AgentGatewayContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (AgentGatewayContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = AgentGatewayConfig::make_feature($name);
    ...
    private static function buildUrl(AgentGatewaySpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof AgentGatewayError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new AgentGatewayResponse($fetched);
        }
    ...
    private function fail(AgentGatewayContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (AgentGatewayContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
