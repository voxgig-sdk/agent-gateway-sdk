...
|---|---|---|
| `ts/` | @voxgig-sdk/agent-gateway-sdk | `.github/workflows/publish-ts.yml` |
...
`ts` target it runs:
    ...
    npm trust github @voxgig-sdk/agent-gateway-sdk \
      --repository voxgig-sdk/agent-gateway-sdk \
      --file publish-ts.yml \
      --allow-publish
...
`npm trust list @voxgig-sdk/agent-gateway-sdk` shows the configuration, and
`npm trust revoke @voxgig-sdk/agent-gateway-sdk --id=<id>` removes one.
