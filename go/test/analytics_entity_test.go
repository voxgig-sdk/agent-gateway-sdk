...
func TestAnalyticsEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set AGENT_GATEWAY_TEST_ANALYTICS_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func analyticsBasicSetup(extra map[string]any) *entityTestSetup {
	...
	idmap := vs.Transform(
		[]any{"analytics01", "analytics02", "analytics03"},
		map[string]any{
	...
	entidEnvRaw := os.Getenv("AGENT_GATEWAY_TEST_ANALYTICS_ENTID")
	...
	idmapResolved := core.ToMapAny(env["AGENT_GATEWAY_TEST_ANALYTICS_ENTID"])
