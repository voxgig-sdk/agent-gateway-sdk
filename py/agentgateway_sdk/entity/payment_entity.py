...
class PaymentEntity:
    ...
    def load(self, reqmatch=None, ctrl=None) -> Payment:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.Payment().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
    ...
    def create(self, reqdata: PaymentCreateData, ctrl=None) -> Payment:
        utility = self._utility
        ctx = utility.make_context({
            "opname": "create",
            "ctrl": ctrl,
