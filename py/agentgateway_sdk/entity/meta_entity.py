...
from agentgateway_sdk.agentgateway_types import (
    Meta,
    MetaLoadMatch,
)
...
class MetaEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return MetaEntity(self._client, opts)
    ...
    def load(self, reqmatch=None, ctrl=None) -> Meta:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.Meta().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
