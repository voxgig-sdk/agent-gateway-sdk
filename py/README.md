...
The Python SDK for the AgentGateway API. Provides an entity-oriented interface following Pythonic conventions.
...
```python
result, err = client.Analytics(None).load({"id": "example_id"}, None)
if err:
    raise Exception(err)
print(result)
```
...
result, err = client.AgentGateway(None).load(
    {"id": "test01"}, None
)
