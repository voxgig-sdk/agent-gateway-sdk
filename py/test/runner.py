...
class AgentGatewayTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            AgentGatewayTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = AgentGatewayTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = AgentGatewayTestRunner.getenv("AGENT_GATEWAY_TEST_LIVE")
        override = AgentGatewayTestRunner.getenv("AGENT_GATEWAY_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = AgentGatewayTestRunner.getenv(key)
        ...
        explain = AgentGatewayTestRunner.getenv("AGENT_GATEWAY_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if AgentGatewayTestRunner._test_control is not None:
            return AgentGatewayTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                AgentGatewayTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            AgentGatewayTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = AgentGatewayTestRunner.load_test_control()
...
def load_env_local():
    AgentGatewayTestRunner.load_env_local()


def env_override(m):
    return AgentGatewayTestRunner.env_override(m)


def entity_data(v):
    return AgentGatewayTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return AgentGatewayTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return AgentGatewayTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return AgentGatewayTestRunner.load_test_control()


def live_client_options():
    return AgentGatewayTestRunner.live_client_options()


def live_delay_ms():
    return AgentGatewayTestRunner.live_delay_ms()
