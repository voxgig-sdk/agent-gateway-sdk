...
class TestPaymentDirect:
    ...
    def test_should_direct_load_payment(self):
        setup = _payment_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-payment", "live" if setup["live"] else "unit")
        if _skip:
