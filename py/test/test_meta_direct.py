...
class TestMetaDirect:

    def test_should_direct_load_meta(self):
        setup = _meta_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-meta", "live" if setup["live"] else "unit")
        if _skip:
...
def _meta_direct_setup(mockres):
    ...
    live = env.get("AGENT_GATEWAY_TEST_LIVE") == "TRUE"
