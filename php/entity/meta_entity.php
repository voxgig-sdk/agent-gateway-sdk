...
class MetaEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new MetaEntity($this->_client, $opts);
    }

    /**
     * @param Meta|array $args Meta data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = AgentGatewayHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return Meta|array The current Meta data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of Meta fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = AgentGatewayHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of Meta fields).
     */
     ...
     * Load a single Meta.
     *
     * @param MetaLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed MetaLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Meta|array The loaded Meta as an assoc-array at the
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = AgentGatewayHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
