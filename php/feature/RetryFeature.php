...
// AgentGateway SDK retry feature
...
class AgentGatewayRetryFeature extends AgentGatewayBaseFeature
{
    ...
    public function init(AgentGatewayContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (AgentGatewayContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(AgentGatewayContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(AgentGatewayContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
