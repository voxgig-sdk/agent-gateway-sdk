...
class MetaEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = AgentGatewaySDK::test(null, null);
        $ent = $testsdk->Meta(null);
        $this->assertNotNull($ent);
    }

    public function test_basic_flow(): void
    {
        $setup = meta_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["load"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "meta." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set AGENT_GATEWAY_TEST_META_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $meta_ref01_data = null;
        if (count($meta_ref01_data_raw) > 0) {
...
function meta_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/meta/MetaTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["meta01", "meta02", "meta03"] as $k) {
    ...
    $entid_env_raw = getenv("AGENT_GATEWAY_TEST_META_ENTID");
