...
function envOverride(m: Record<string, any>) {
  if (
    'TRUE' === process.env.AGENTGATEWAY_TEST_LIVE ||
    'TRUE' === process.env.AGENTGATEWAY_TEST_OVERRIDE
  ) {
  ...
  m.AGENTGATEWAY_TEST_EXPLAIN = process.env.AGENTGATEWAY_TEST_EXPLAIN || m.AGENTGATEWAY_TEST_EXPLAIN
