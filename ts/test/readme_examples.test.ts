...
//     (\`new AgentGatewaySDK(...)\` / \`AgentGatewaySDK.test(...)\` become
//     \`AgentGatewaySDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'AgentGatewaySDK'
...
const TEST_SEED = {"entity":{"analytics":{"test01":{"id":"test01"}},"api_key":{"test01":{"id":"test01"}},"balance":{"test01":{"id":"test01"}},"meta":{"test01":{"id":"test01"}},"payment":{"test01":{"id":"test01"}},"service":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new AgentGatewaySDK(...)\` and any
// \`AgentGatewaySDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+AgentGatewaySDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/AgentGatewaySDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
