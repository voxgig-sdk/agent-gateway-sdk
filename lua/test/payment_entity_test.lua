...
describe("PaymentEntity", function()
  ...
  it("should run basic flow", function()
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set AGENT_GATEWAY_TEST_PAYMENT_ENTID JSON to run live")
      return
    end
    ...
    assert.is_nil(err)
    assert.is_not_nil(payment_ref01_data_dt0_loaded)
...
function payment_basic_setup(extra)
  ...
  local entid_env_raw = os.getenv("AGENT_GATEWAY_TEST_PAYMENT_ENTID")
