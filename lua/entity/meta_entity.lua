...
local MetaEntity = {}
MetaEntity.__index = MetaEntity


function MetaEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, MetaEntity)
  self._name = "meta"
  self._client = client
...
function MetaEntity:get_name()
  return self._name
end


function MetaEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return MetaEntity.new(self._client, opts)
end
...
function MetaEntity:mark_deleted()
  self._deleted = true
end


function MetaEntity:deleted()
  return true == self._deleted
end


function MetaEntity:data_set(args)
  if args ~= nil then
...
function MetaEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function MetaEntity:match_set(args)
  if args ~= nil then
...
function MetaEntity:stream(action, args, callopts)
...
---@param reqmatch MetaLoadMatch
---@param ctrl? table
---@return Meta
---@return string? err
function MetaEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function MetaEntity:_run_op(ctx, post_done)
  local utility = self._utility
