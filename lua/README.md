# AgentGateway Lua SDK

The Lua SDK for the AgentGateway API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:Analytics(nil):load({ id = "example_id" }, nil)
...
local result, err = client:AgentGateway(nil):load(
  { id = "test01" }, nil
)
...
The main module (`agent-gateway_sdk`) exports the SDK constructor
